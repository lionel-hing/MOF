...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, config, data_loader,
        ...
        super().__init__(model, loss, metrics, config, writer)
        self.config = config
    ...
    @contextmanager
    def unflatten_weight(self, flat_w, is_saved):
        ws = (t.view(s) for (t, s) in zip(flat_w.split(self.model._weights_numels), self.model._weights_shapes))
        for (m, n), w in zip(self.model._weights_module_names, ws):
            setattr(m, n, w)
        yield
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(zip(*self.data_loader), desc=f"Training model epoch {epoch}", total=total_iterations) as progress:
            for batch_idx, data_li in enumerate(progress):
                if (batch_idx + 1) * self.total_batch_sum > self.max_samples_per_epoch:
                    break
                for dl_idx, data in enumerate(data_li):  # data (32 8 3 244 244)
                    ...
                    for i in range(len(data['video_id'])):
                        video_id = int(data['video_id'][i][5:])
                        video_params_list.append(self.params[video_id].unsqueeze(0))
                    video_params = torch.cat(video_params_list).to(self.device)  

                    video_params.requires_grad = True
                   
                    del video_params_list

                    self.optimizer.zero_grad()

                    with self.unflatten_weight(self.model.flat_w, False):
                        text_embeds, video_embeds = self.model(data['text'], video_params)
        ...
        if epoch % 1 == 0:
            train_video_log = self._train_video_epoch(epoch)
            log.update(train_video_log)
    ...
    def _train_video_epoch(self, epoch):
        ...
        total_loss = [0] * len(self.data_loader)
        total_iterations = self.max_samples_per_epoch // self.total_batch_sum + 1
        with tqdm(zip(*self.data_loader), desc=f"Training video epoch {epoch}", total=total_iterations) as progress:
            for batch_idx, data_li in enumerate(progress):
                if (batch_idx + 1) * self.total_batch_sum > self.max_samples_per_epoch:
                    break
                for dl_idx, data in enumerate(data_li):
                    ...
                    if self.tokenizer is not None:
                        data['text'] = self.tokenizer(data['text'], return_tensors='pt', padding=True,
                                                      truncation=True)
                    data['text'] = {key: val.to(self.device) for key, val in data['text'].items()}
                    data['video'] = data['video'].to(self.device)  # （9000，8， 3， 224，224）

                    text_data = data['text']
                    raw_video_data = data['video']
                    ...
                    video_params.requires_grad = True
                    video_params.grad = torch.zeros_like(video_params)

                    del video_params_list

                    # 1. compute loss
                    with self.unflatten_weight(w, False):
                        text_embeds, video_embeds = self.model(text_data, video_params)
                    output = sim_matrix(text_embeds, video_embeds)
                    loss = self.loss(output)

                    # 2. compute loss grad
                    gw, = (torch.autograd.grad(loss, list(self.model.parameters()), create_graph=True))
                    ...
                    # 4. compute loss2
                    # forward_with_param
                    with self.unflatten_weight(new_w, False):
                        text_embeds, raw_video_embeds = self.model(text_data, raw_video_data)
                    output2 = sim_matrix(text_embeds, raw_video_embeds)
                    loss2 = self.loss(output2)

                    # 5. compute loss2 grad
                    dw, = torch.autograd.grad(loss2, new_w)

                    dgw = dw.neg()

                    # 6. compute video_params grad
                    d_grad, = torch.autograd.grad(outputs=(gw,), inputs=video_params, grad_outputs=(dgw,))

                    with torch.no_grad():
                        for i in range(len(data['video_id'])):
                            video_id = int(data['video_id'][i][5:])
                            self.params[video_id].grad.add_(d_grad[i].cpu())
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdm(dl, desc=f"Validating dl{dl_idx}"):
                    ...
                    if avoid_data_parallel:
                        with self.unflatten_weight(self.model.flat_w, False):
                            text_embed, vid_embed = self.model.module(data['text'], data['video'] )
                    else:
                        with self.unflatten_weight(self.model.flat_w, False):
                            text_embed, vid_embed = self.model(data['text'], data['video'])
